  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={1} mb={3}>
        ...
        <Typography variant="h5" color="primary.main" fontWeight="bold">Clientes</Typography>
      </Box>

      <Paper elevation={2} sx={{ p: 3, borderTop: 4, borderColor: 'primary.main' }}>
        <Typography variant="subtitle1" fontWeight="bold" mb={2}>Agregar nuevo cliente</Typography>
        <Box component="form" onSubmit={handleAddClient} sx={{ display: 'flex', gap: 2, mb: 4, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <TextField
            label="Nombre de la Empresa" size="small" required
            value={newClient.name} onChange={e => setNewClient({ ...newClient, name: e.target.value })}
            sx={{ flexGrow: 2, minWidth: 200 }}
          />
          <TextField
            label="Contacto" size="small"
