...
const PurchaseOrder: React.FC = () => {
  ...
  const MetaItem: React.FC<{ label: string; value: string; span?: number }> = ({ label, value }) => (
    ...
    <Box sx={{ display: 'flex', justifyContent: 'center' }}>
      <Paper elevation={3} sx={{ p: 4, width: '100%', maxWidth: 1000, bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, borderBottom: 2, borderColor: 'black', pb: 1 }}>
          <Typography variant="h4"><Box component="span" sx={{ fontWeight: 900 }}>Royalapparel</Box> PURCHASE ORDER</Typography>
        </Box>
        <Typography variant="body2" sx={{ fontWeight: 'bold', mb: 2 }}>* ORIGINAL ORDER *</Typography>

        <Grid container spacing={2} mb={3}>
          <Grid item xs={12} md={4}>
            <InfoBox title="TO:" content={`400427\nTextilera Nicaragüense Interna\nKm 5.5 Carretera Norte\nZip Argeñal, ni\n\nATTN: Roywei Chen\nPHONE: 505-224-9-36\nFAX:`} />
          </Grid>
          <Grid item xs={12} md={4}>
            <InfoBox title="BILL TO:" content={`Royal Apparel\n91 Cabot Ct\nHauppauge NY 11788\n\n\n\nPHONE: 866-769-2517\nFAX:`} />
          </Grid>
          <Grid item xs={12} md={4}>
            <InfoBox title="SHIP TO:" content={`Royal Apparel - Long Island\n91 Cabot Court\nHauppauge, NY 11788\n\n\n\nATTN:`} />
          </Grid>
        </Grid>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
          <Box sx={{ bgcolor: 'black', color: 'white', px: 1, py: 0.5 }}>
            <Typography variant="caption" sx={{ fontWeight: 'bold' }}>ADDITIONAL COMMENTS:</Typography>
          </Box>
        ...
        <Typography variant="body2" sx={{ fontWeight: 'bold', mt: 2, mb: 2 }}>***** Amounts Specified in USD *****</Typography>
        <Typography variant="body2" sx={{ fontWeight: 'bold', mb: 3 }} color="error">OUR P/O NUMBER MUST APPEAR ON ALL PACKAGES, INVOICES AND CORRESPONDENCE.</Typography>

        <Box sx={{ border: 1, borderColor: 'grey.300', p: 2 }}>
          <Typography variant="caption" sx={{ fontWeight: 'bold', display: 'block', mb: 1 }}>TERMS AND CONDITIONS OF THIS ORDER:</Typography>
        ...
        <Typography variant="caption" display="block" textAlign="right" mt={2}>Page 1 of 1</Typography>
      </Paper>
    </Box>
  );
};
