...
import { usePpSamples } from '../../hooks/ppSamples/usePpSamples';
...
const STATUS_OPTIONS = [
  'Not Started', 'In Preparation', 'Internal Review', 'Submitted to Customer',
  'Approved', 'Approved with Comments', 'Rejected', 'Resubmission Required', 'On Hold',
];

const sc = (s: string) => {
  const m: Record<string, any> = { Approved: 'success', Passed: 'success', Rejected: 'error', Failed: 'error', 'On Hold': 'warning', Pending: 'warning', 'Not Started': 'default' };
  return m[s] ?? 'default';
};

const emptyForm = {
  FGPOId: 0, SizeId: 0, SampleVersion: 'V1', FabricLot: '', TrimVersion: '',
  PreparationDate: '', SubmissionDate: '',
  MeasurementResult: 'Pending', ConstructionResult: 'Pending', FitResult: 'Pending',
  FabricResult: 'Pending', TrimResult: 'Pending', LabelResult: 'Pending',
  InternalReview: 'Pending', CustomerReview: 'Pending', CustomerComments: '',
  ApprovalDate: '', ApprovedByUserId: 0, Status: 'Not Started',
...
const PPSamplePage: React.FC = () => {
  ...
  const [viewItem, setViewItem] = useState<PpSample | null>(null);
  ...
  const openEdit = (item: PpSample) => {
    setEditingId(item.id);
    setForm({
      FGPOId: item.fgpoId, SizeId: sizeList.find(o => o.label === item.size)?.id ?? 0, SampleVersion: item.sampleVersion || 'V1',
      FabricLot: item.fabricLot ?? '', TrimVersion: item.trimVersion ?? '',
      PreparationDate: item.preparationDate?.split('T')[0] || '',
      ...
      FitResult: item.fitResult || 'Pending',
      FabricResult: item.fabricResult || 'Pending',
      TrimResult: item.trimResult || 'Pending',
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        PreparationDate: form.PreparationDate ? new Date(form.PreparationDate).toISOString() : null,
        SubmissionDate: form.SubmissionDate ? new Date(form.SubmissionDate).toISOString() : null,
  ...
  const resultFields: { key: keyof typeof emptyForm; label: string }[] = [
    ...
    { key: 'FitResult', label: 'Fit' },
    { key: 'FabricResult', label: 'Fabric' },
    { key: 'TrimResult', label: 'Trim' },
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>
            <StyleIcon sx={{ mr: 1, verticalAlign: 'middle', color: 'primary.main' }} />
            PP Sample
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            Pre-Production Sample — muestra inicial para aprobación de cliente
          </Typography>
        </Box>
      ...
      <Paper sx={{ mb: 2, p: 1.5, display: 'flex', gap: 1 }}>
        <Box component="form" onSubmit={handleSearch} sx={{ display: 'flex', gap: 1, flex: 1 }}>
          <TextField size="small" placeholder="Search by FGPO, Version, Fabric Lot, Status..."
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ backgroundColor: 'primary.main' }}>
                {['FGPO', 'Style', 'Color', 'Size', 'Version', 'Fabric Lot', 'Prep Date', 'Submit Date', 'Fit', 'Fabric', 'Status', 'Actions'].map(h =>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={12} align="center" sx={{ py: 6, color: 'text.secondary' }}>
                  <Typography variant="body1">No PP samples found</Typography>
                  <Button variant="text" onClick={openCreate} sx={{ mt: 1 }}>Create your first sample</Button>
                </TableCell></TableRow>
              ) : items.map((item: PpSample) => (
                <TableRow key={item.id} hover>
                  <TableCell>{item.fgpoNumber}</TableCell>
                  <TableCell>{item.style || '-'}</TableCell>
                  <TableCell>{item.color || '-'}</TableCell>
                  <TableCell>{item.size || '-'}</TableCell>
                  <TableCell>{item.sampleVersion || '-'}</TableCell>
                  <TableCell>{item.fabricLot || '-'}</TableCell>
                  <TableCell>{fmt(item.preparationDate)}</TableCell>
                  <TableCell>{fmt(item.submissionDate)}</TableCell>
                  <TableCell><Chip label={item.fitResult || 'N/A'} size="small" color={sc(item.fitResult ?? '')} variant="outlined" /></TableCell>
                  <TableCell><Chip label={item.fabricResult || 'N/A'} size="small" color={sc(item.fabricResult ?? '')} variant="outlined" /></TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="lg" fullWidth>
        ...
        <DialogContent sx={{ pt: 3, pb: 1 }}>
          ...
          <Grid container spacing={2.5}>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4 }}><TextField fullWidth size="small" label="Sample Version" value={form.SampleVersion} onChange={e => setF('SampleVersion', e.target.value)} /></Grid>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4 }}><TextField fullWidth size="small" label="Preparation Date" type="date" value={form.PreparationDate} onChange={e => setF('PreparationDate', e.target.value)} slotProps={{ inputLabel: { shrink: true } }} /></Grid>
            <Grid size={{ xs: 12, sm: 6, md: 4 }}><TextField fullWidth size="small" label="Submission Date" type="date" value={form.SubmissionDate} onChange={e => setF('SubmissionDate', e.target.value)} slotProps={{ inputLabel: { shrink: true } }} /></Grid>
            ...
            <Grid size={{ xs: 12 }}><TextField fullWidth size="small" label="Customer Comments" value={form.CustomerComments} onChange={e => setF('CustomerComments', e.target.value)} multiline rows={2} /></Grid>
        ...
        <DialogContent sx={{ pt: 3 }}>
          {viewItem && (
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Size / Version</Typography><Typography>{viewItem.size || '-'} / {viewItem.sampleVersion || '-'}</Typography></Grid>
              <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Fabric Lot / Trim Version</Typography><Typography>{viewItem.fabricLot || '-'} / {viewItem.trimVersion || '-'}</Typography></Grid>
              <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Preparation Date</Typography><Typography>{fmt(viewItem.preparationDate)}</Typography></Grid>
              <Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption" color="text.secondary">Submission Date</Typography><Typography>{fmt(viewItem.submissionDate)}</Typography></Grid>
              ...
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Fit</Typography><Chip label={viewItem.fitResult || 'N/A'} size="small" color={sc(viewItem.fitResult ?? '')} /></Grid>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Fabric</Typography><Chip label={viewItem.fabricResult || 'N/A'} size="small" color={sc(viewItem.fabricResult ?? '')} /></Grid>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Trim</Typography><Chip label={viewItem.trimResult || 'N/A'} size="small" color={sc(viewItem.trimResult ?? '')} /></Grid>
              ...
              {viewItem.customerComments && <Grid size={{ xs: 12 }}><Divider sx={{ my: 0.5 }} /><Typography variant="caption" color="text.secondary">Customer Comments</Typography><Typography>{viewItem.customerComments}</Typography></Grid>}
            </Grid>
          )}
        </DialogContent>
